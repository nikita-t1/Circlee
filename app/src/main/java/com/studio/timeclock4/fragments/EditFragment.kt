...
class EditFragment(private val workDay: WorkDay, private val databaseAction: DatabaseAction) :
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (workDay != null) {
            ...
            date.text =
                "${workDay.dayOfMonth}. ${Month.of(workDay.month).capitalize()} ${workDay.year}"
        } else {
    ...
    override fun onResume() {
        ...
        calcWorkTime()
        if (databaseAction == DatabaseAction.PREVIEW) workTimeString.text =
            TC.convertMinutesToNegativeDateString(end - start - pause)
    }
    ...
    override fun onClick(v: View?) {
        ...
        when (v) {
            saveBtn -> {
                ...
                newWorkDay.apply {
                    timeClockIn =
                        TC.convertDateStringToMinutes(startTimeString.text.toString()).toInt()
                    timeClockOut =
                        TC.convertDateStringToMinutes(endTimeString.text.toString()).toInt()
                    pauseTime =
                        TC.convertDateStringToMinutes(pauseTimeString.text.toString()).toInt()
                    ...
                    if (databaseAction == DatabaseAction.UPDATE) listingViewModel.updateWorkDay(
                        newWorkDay
                    )
                    else if (databaseAction == DatabaseAction.INSERT) listingViewModel.insertWorkDay(
                        newWorkDay
                    )
            ...
            startCard -> {
                ...
                dialog.setListener { hour, minute ->
                    dialogView.findViewById<TextView>(R.id.startTimeString)?.text =
            ...
            endCard -> {
                ...
                dialog.setListener { hour, minute ->
                    dialogView.findViewById<TextView>(R.id.endTimeString)?.text =
                        constructDateString(hour, minute)
                    end = TC.convertDateStringToMinutes(endTimeString.text.toString())
                    calcWorkTime()
                }
