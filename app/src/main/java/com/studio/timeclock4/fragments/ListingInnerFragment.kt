        ...
        when (v) {
            edit_btn -> {
                ...
                val dialog = EditFragment(
                    listingViewModel.weekDayToViewModelElement(
                        DayOfWeek.of(
                            dayToInt(lastClickedButton!!.text.toString())
                        )
                    ).value!!, EditFragment.DatabaseAction.UPDATE
                )
                ...
                listingViewModel.viewModelScope.launch(Dispatchers.Main) {
                    ...
                    if (PreferenceHelper.read(
                            PreferenceHelper.DEV_EnableDayButtonAnimation,
                            false
                        )
                    ) {
                    ...
                    date_full_text.text = "${currentDate.dayOfMonth}. ${currentDate.month.toString()
                        .capitalise()} ${currentDate.year}"
                    day_text.text = currentDate.dayOfWeek.toString().capitalise()
                    day_balance_current_text.text =
                        convertMinutesToDateString(workday.value!!.workTimeGross.toLong())
                    day_balance_remaining_text.text =
                        convertMinutesToDateString(workday.value!!.overtime.toLong())
                    starttime_text.text =
                        convertMinutesToDateString(workday.value!!.timeClockIn.toLong())
                    endtime_text.text =
                        convertMinutesToDateString(workday.value!!.timeClockOut.toLong())
                    breaktime_text.text =
                        convertMinutesToDateString(workday.value!!.pauseTime.toLong())
