        ...
        materialStandardPreference6.apply {
            ...
            materialStandardPreference2.setOnClickListener() {
                Navigation.findNavController(view)
                    .navigate(R.id.action_destination_options_to_destination_timeSettingsFragment)
            }

            setOnClickListener() {
                AboutFragment()
                Navigation.findNavController(view)
                    .navigate(R.id.action_destination_options_to_destination_about)
            }

            setOnLongClickListener {
                if (BuildConfig.DEBUG) {
                    Navigation.findNavController(view)
                        .navigate(R.id.action_destination_options_to_destination_settings)
                } else {
                    val i =
                        resources.openRawResource(R.raw.todo).bufferedReader().use { it.readText() }
                    BottomSheetFragment(
