...
//            value = "${(Pref.read(Pref.WORKING_TIME, Pref.Default_WORKING_TIME).toFloat() / 60f).toInt()}.${(Pref.read(Pref.WORKING_TIME, 480L).toFloat() % 60f /60*100).toInt()} $hourShort"
            value = TimeCalculations.convertMinutesToDateString(
                Pref.read(
                    Pref.WORKING_TIME,
                    Pref.Default_WORKING_TIME
                )
            )
        }
        _workingTimeWeek.apply {
            value = "${(Pref.read(Pref.WORKING_TIME_WEEK, Pref.Default_WORKING_TIME_WEEK)
                .toFloat() / 60f).toInt()}." +
                    "${(Pref.read(Pref.WORKING_TIME_WEEK, Pref.Default_WORKING_TIME_WEEK)
                        .toFloat() % 60f / 60 * 100).toInt()} $hourShort"
        }
        _pauseTime.apply {
            value = TimeCalculations.convertMinutesToDateString(
                Pref.read(
                    Pref.PAUSE_TIME,
                    Pref.Default_PAUSE_TIME
                )
            )
        }
        _flexAccount.apply {
            value = "${(Pref.read(Pref.FLEX_ACCOUNT, Pref.Default_FLEX_ACCOUNT)
                .toFloat() / 60f).toInt()}." +
                    "${(Pref.read(Pref.FLEX_ACCOUNT, Pref.Default_FLEX_ACCOUNT)
                        .toFloat() % 60f / 60 * 100).toInt()} $hourShort"
        }
        _vacation.apply {
        ...
        if (!text.isBlank() && !text.startsWith(".") && !text.endsWith(".")) {
            if (field != InputField.vacation) {
                ...
                min = if (isFloat) {
                    ...
                    (parts[0].toInt() * 60 +
                            (ceil(
                                (60 * (parts[1].toInt())).toDouble() / (10.toDouble()
                                    .pow(parts[1].length))
                            ))
                            ).toInt()
                } else {
            ...
            when (field) {
                ...
                InputField.workingTimeWeek -> updateField(
                    ...
                    ceil(
                        min / Pref.read(Pref.WORKING_DAYS_WEEK, Pref.Default_WORKING_DAYS_WEEK)
                            .toDouble()
                    ).toLong()
                )
            }
    ...
    fun updateWorkWeek(dayChip: String, isChecked: Boolean) {
        ...
        Timber.e(
            "(Pref.read(Pref.WORKING_TIME, Pref.Default_WORKING_TIME) ${(Pref.read(
                Pref.WORKING_TIME,
                Pref.Default_WORKING_TIME
            ))} "
        )
        updateField(
            InputField.workingTimeWeek,
            (Pref.read(Pref.WORKING_TIME, Pref.Default_WORKING_TIME) * workingDaysAmount)
        )
    }
}
