...
class HomeViewModel(application: Application) : AndroidViewModel(application), LifecycleObserver {
    ...
    fun onCreate() {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            if (workingDaysWeek > daysInDatabaseWeek.size) {
                ...
                meanWorkTimeSetPoint =
                    workDaySetPoint / (daysInDatabaseWeek.size + missingDays) * workingDaysWeek
            } else {
            ...
            withContext(Dispatchers.Main) {
                _progressBarWeek.value = kotlin.math.floor(
                    ((workTimeNetWeek) / meanWorkTimeSetPoint.toFloat()) * 100
                ).toInt()
            }
        }
    }
    ...
    init {
        ...
        pauseTimeMin =
            Pref.read(Pref.CURRENT_PAUSE_TIME, Pref.read(Pref.PAUSE_TIME, Pref.Default_PAUSE_TIME))
        _pauseTime.apply {
    ...
    private fun updateLayoutState() {
        ...
        when (currentLayoutStateOrdinal.value) {
            ...
            LayoutState.Tracking -> {
                ...
                startTimeMin = Pref.read(Pref.CURRENT_START_TIME, Pref.Default_START_TIME)
                pauseTimeMin = Pref.read(
                    Pref.CURRENT_PAUSE_TIME,
                    Pref.read(Pref.PAUSE_TIME, Pref.Default_PAUSE_TIME)
                )
                noteString = Pref.read(Pref.CURRENT_NOTE, Pref.Default_CURRENT_NOTE)

                endTimeMin =
                    TimeCalculations.loadEndTime(startTimeMin, workingTimeMin, pauseTimeMin, true)
    ...
    fun onChronometerClick(elapsedMillis: Long, isHourFormat: Boolean) {
        ...
        _progressBarDay.value =
            kotlin.math.floor((elapsedMin / (workingTimeMin + pauseTimeMin) * 100)).toInt()
        ...
        _progressBarWeek.value =
            kotlin.math.floor(((workTimeNetWeek + elapsedMin.toInt()) / meanWorkTimeSetPoint.toFloat()) * 100)
                .toInt()
    ...
    fun dialogSave(endTime: Long) {
        viewModelScope.launch {
            ...
            Timber.e(
                "workDay.toString()  ${workDay.toString().length} ${workDay.toString()
                    .subSequence(0, 16)}"
            )
            workDay.toString()
            if (Pref.read(Pref.DEV_EnableSaving, true) or !BuildConfig.DEBUG) {
                val existingWorkDay =
                    repository.getWorkday(ldt.dayOfMonth, ldt.monthValue, ldt.year)
