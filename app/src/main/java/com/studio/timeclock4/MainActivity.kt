        ...
        navController.addOnDestinationChangedListener { controller, destination, arguments ->
            if (BuildConfig.DEBUG) {
                ...
                when (PreferenceHelper.read(PreferenceHelper.DEV_ColorTitle_U, false)) {
                    true -> ss.setSpan(
                        ForegroundColorSpan(
                            PreferenceHelper.read(
                                PreferenceHelper.DEV_TitleColor,
                                0
                            )
                        ), 0, 1, 0
                    )
            ...
            if (destination.id == R.id.destination_listing) {
                ...
                window.statusBarColor =
                    resources.getColor(R.color.alpha, null)     //Transparent Background
            ...
            if (destination.id == R.id.destination_options) findViewById<TextView>(R.id.toolbar_title).text =
