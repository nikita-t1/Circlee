...
class ChronometerPersist private constructor() {
    ...
    private fun startStateChronometer() {
        ...
        Timber.i(
            "KEY_BASE + mChronometer.id Read: ${PreferenceHelper.read(
...
//            mChronometer.text = "00:00"
    ...
    private fun saveBase() {
        Timber.i(
            "KEY_BASE + mChronometer.id Write before: ${PreferenceHelper.read(
                KEY_BASE + mChronometer.id,
                10L
            )}"
        )
        ...
        Timber.i(
            "KEY_BASE + mChronometer.id Write after: ${PreferenceHelper.read(
                KEY_BASE + mChronometer.id,
                10L
            )}"
        )
    ...
    fun substractFromChronometerBase(sec: Long) {
        val ohYeah =
            PreferenceHelper.read(KEY_BASE + mChronometer.id, SystemClock.elapsedRealtime())
